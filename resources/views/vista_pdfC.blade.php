...
<html>
<head>
    <title>CONSTANCIA DE CONTRUCCION</title>
    <style>
...
<body>
    ...
    <p>
        El Aporte Estatal otorgado corresponde a la Categoría <strong> {{ $task->category->name }} </strong>, el cual asciende a la suma de Gs <strong> {{ number_format((int)(($task->amount * $task->category->percentage) / 100),0,".",".")  }} </strong>, equivalente al
       <strong> {{$task->category->percentage}} % </strong> de monto total del proyecto constructivo recibido en relación al inmueble individualizado como
       @if ($task->farm)
       Finca o Matrícula N° <strong> {{ $task->farm }} </strong>
       @endif
       Cta. Cte. Ctral o Padrón N° <strong> {{ $task->account }} </strong>
